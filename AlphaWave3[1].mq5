...
//Enter new Positions based on the number of sells and buys
   ...
   if(buyCount > sellCount && ifBidIsinRange(bid) && TradeOnce == false)
     {
      ...
      if(sellCount > buyCount && ifBidIsinRange(bid) && TradeOnce == false)
        {
         ...
         if(ifBidIsinRange(bid) && buyCount == sellCount && GridCreateOnce == false && TradeOnce == false)
           {
   ...
   for(int i=0;i<ArraySize(closedTrades);i++)
     {
      if(bid == closedTrades[i].entryPrice)
        {
         Print("rightnow bid is on one of the grid lines");
         //DeleteFromArray(positionEntries, positionEntries[i]);
         //arraySize = ArraySize(positionEntries);
         //for(int i=0;i<ArraySize(positionEntries);i++)
         //  {
         //   Print("Remaining Entries: ",positionEntries[i]);
         //  }
         return true;
         TradeOnce = false;
        }
